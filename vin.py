...
def lexical_analyzer(text, file_path):
    """Лексический анализатор с детальной проверкой."""
    ...
    def clean_and_report_invalid_fragments(text):
        ...
        def is_valid_char(c):
            return c.isalnum() or c == '_' or c in 'аА'
        ...
        while i < len(text):
            ...
            if c in '+-':
                if i + 1 < len(text) and text[i + 1].isdigit():
                    buffer += c  # допустимый знак перед числом
                    i += 1
                    buffer += text[i]  # первая цифра
                    i += 1
            ...
            if is_valid_char(c):
                ...
                if c.isspace() or c in ":=;":
                    result += c  # допустимые символы
                else:
                    # Собираем недопустимый фрагмент
                    invalid_segment = ''
    ...
    # --- Проверка ключевых слов, структуры, идентификаторов и чисел ---
    def check_keywords_and_report_errors(cleaned_text):
        ...
        if has_equal:
            ...
            if len(parts) > 1:
                value_token = parts[1].strip().split()[0]
        else:
        ...
        for symbol in required_symbols:
            if symbol == '=' and not has_equal:
                continue
            if symbol not in cleaned_text:
                errors.append(f"нет токена ({symbol})")

        if value_token and not re.fullmatch(r'[+-]?\d+', value_token):
            errors.append(f"неверный фрагмент в числе ({value_token})")
